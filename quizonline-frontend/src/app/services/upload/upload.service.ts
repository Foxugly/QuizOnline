 ...
 * ``PATCH /api/v1/block/{id}/`` with a single file field, which the
 ...
 * :class:`CatalogService` pattern: the editors do their own toast
...
export class UploadService {
  ...
  private buildUrl(blockId: number): string {
    return `${resolveApiBaseUrl().replace(/\/+$/, '')}/api/v1/block/${blockId}/`;
  }
