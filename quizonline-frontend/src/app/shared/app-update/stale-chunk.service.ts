...
export class StaleChunkService {
  ...
  // MessageService directement plutot que la facade AppToastService : celle-ci
  // n'expose pas ``sticky``, et ce message ne doit surtout pas disparaitre tout
  // seul au bout de 4 s — c'est le seul indice que l'utilisateur recevra.
  ...
  private static readonly RELOAD_MARKER = 'quizonline.staleChunkReloadAt';
  ...
  private offerReload(): void {
    this.messages.add({
      ...
      sticky: true,
      closable: true,
      summary: this.uiText.ui().appUpdate.title,
      detail: this.uiText.ui().appUpdate.detail,
    });
  }
