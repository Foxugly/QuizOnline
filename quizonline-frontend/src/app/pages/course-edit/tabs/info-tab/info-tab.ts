...
import {getCourseEditInfoTabUiText} from './info-tab.i18n';
 ...
 * block editors (see :class:`UploadService`): the form first PATCHes
 ...
 */
@Component({
  selector: 'app-course-edit-info-tab',
...
export class CourseEditInfoTab {
  // ---- DI ------------------------------------------------------------------
  ...
  protected readonly pageText = this.uiSvc.localized(getCourseEditInfoTabUiText);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      // ``/course/:id/edit`` URLs without a full re-route) or after
