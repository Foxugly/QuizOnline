...
echo ""
echo "Enable daily database backups (recommended):"
echo "  sudo install -m 644 $REPO_DIR/deploy/quizonline-backup.service /etc/systemd/system/"
echo "  sudo install -m 644 $REPO_DIR/deploy/quizonline-backup.timer   /etc/systemd/system/"
echo "  sudo install -d -o django -g www-data -m 700 /var/backups/quizonline"
echo "  sudo systemctl daemon-reload && sudo systemctl enable --now quizonline-backup.timer"
echo "  # Optional: edit the .service to set S3_BUCKET= for an off-host mirror."
