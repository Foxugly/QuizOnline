...
class QuizViewsAPITestCase(_ReverseMixin, APITestCase):
    ...
    @patch("quiz.services.notify_quiz_completed")
    def test_close_quiz_computes_scores_and_deactivates(self, notify_quiz_completed):
        quiz = self._create_quiz(self.qt_ok, self.u1, active=True, started_at=timezone.now())
        ...
        notify_quiz_completed.assert_called_once_with(quiz)
