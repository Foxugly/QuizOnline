...
GET /api/v1/lesson/{id}/note/  → returns the caller's note or an
                                  empty one if no note exists yet.
PUT /api/v1/lesson/{id}/note/  → upserts the content.
...
from enrollment.models import LessonNote
...
@pytest.fixture
def url(lesson):
    return reverse("api:enrollment-api:lesson-note", args=[lesson.id])
...
def test_404_on_missing_lesson(owner):
    resp = _auth(owner).get(reverse("api:enrollment-api:lesson-note", args=[99999]))
