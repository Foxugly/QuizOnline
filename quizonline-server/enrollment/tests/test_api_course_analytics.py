...
GET /api/v1/course/{id}/analytics/ aggregates KPIs the course-edit
...
@pytest.fixture
def url(course):
    return reverse("api:enrollment-api:course-analytics", args=[course.id])
...
def test_404_when_course_missing(owner):
    resp = _auth(owner).get(reverse("api:enrollment-api:course-analytics", args=[99999]))
