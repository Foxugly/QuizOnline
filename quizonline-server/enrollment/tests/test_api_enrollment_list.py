"""End-to-end tests for ``GET /api/v1/enrollment/`` — verifies the
...
def test_instructor_sees_all_enrollments_for_their_course(course, owner, three_learners):
    """Course owner GET /api/v1/enrollment/?course=<id> → every enrollment row."""
    for learner in three_learners:
        _enroll(learner, course)

    client = APIClient()
    client.force_authenticate(user=owner)
    resp = client.get(f"/api/v1/enrollment/?course={course.id}")
...
def test_status_filter_narrows_results_for_instructor(course, owner, three_learners):
    ...
    client.force_authenticate(user=owner)
    resp = client.get(
        f"/api/v1/enrollment/?course={course.id}&status=pending"
    )
...
def test_invalid_course_filter_returns_empty(course, owner):
    ...
    client = APIClient()
    client.force_authenticate(user=owner)
    resp = client.get("/api/v1/enrollment/?course=not-a-number")
...
def test_unscoped_request_still_returns_own_enrollments(course, three_learners):
    ...
    other users' rows on the plain ``/api/v1/enrollment/`` endpoint."""
    for learner in three_learners:
        _enroll(learner, course)
    ...
    client = APIClient()
    client.force_authenticate(user=me)
    resp = client.get("/api/v1/enrollment/")
