...
def test_section_title_and_position(owner, course_with_two_sections):
    ...
    resp = _auth(owner).get(f"/api/v1/lesson/{s1l2.id}/")
...
def test_neighbours_at_course_start(owner, course_with_two_sections):
    ...
    resp = _auth(owner).get(f"/api/v1/lesson/{course_with_two_sections['s1l1'].id}/")
    assert resp.status_code == 200
    body = resp.json()
...
def test_neighbours_at_section_boundary(owner, course_with_two_sections):
    ...
    resp = _auth(owner).get(f"/api/v1/lesson/{course_with_two_sections['s1l2'].id}/")
    assert resp.status_code == 200
    body = resp.json()
...
def test_neighbours_at_course_end(owner, course_with_two_sections):
    ...
    resp = _auth(owner).get(f"/api/v1/lesson/{course_with_two_sections['s2l2'].id}/")
    assert resp.status_code == 200
    body = resp.json()
...
def test_neighbours_solo_lesson(owner, course):
    ...
    resp = _auth(owner).get(f"/api/v1/lesson/{solo.id}/")
    assert resp.status_code == 200
    body = resp.json()
    assert body["prev_lesson"] is None
    assert body["next_lesson"] is None