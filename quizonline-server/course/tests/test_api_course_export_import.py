...
GET  /api/v1/course/{id}/export/  → dump the course as a JSON dict.
POST /api/v1/course/import/       → recreate the course in a target
...
def test_export_endpoint_instructor_gated(populated_course, db):
    ...
    resp = _auth(stranger).get(f"/api/v1/course/{populated_course.id}/export/")
