"""Coverage for the ``GET /api/v1/course/?manageable_only=1`` filter.
...
def test_manageable_only_filters_to_owned_and_managed_courses(
    ...
    plain = client.get("/api/v1/course/").json()
    ...
    filtered = client.get("/api/v1/course/?manageable_only=1").json()
...
def test_manageable_only_returns_empty_for_pure_member(
    ...
    body = client.get("/api/v1/course/?manageable_only=1").json()
    assert body["count"] == 0
    assert body["results"] == []
