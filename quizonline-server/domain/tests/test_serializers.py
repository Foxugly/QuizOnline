...
class DomainSerializersTestCase(TestCase):
    ...
    def test_domain_write_serializer_update_is_atomic_for_instance_fields(self):
        payload = {
            "allowed_languages": [self.lang_fr.id],
            "translations": {"fr": {"name": "Updated FR", "description": "Updated"}},
            "active": False,
        ...
        serializer = DomainWriteSerializer(instance=self.domain, data=payload, context={"request": request})
        self.assertTrue(serializer.is_valid(), serializer.errors)

        with patch.object(DomainWriteSerializer, "_apply_translations", side_effect=RuntimeError("boom")):
            with self.assertRaises(RuntimeError):
                serializer.save()

        self.domain.refresh_from_db()
        self.assertTrue(self.domain.active)

    def test_apply_translations_updates_translation_rows_without_saving_domain_each_time(self):
        serializer = DomainWriteSerializer(context={"request": self.factory.get("/")})
        domain = Domain.objects.create(owner=self.owner, active=True)
        domain.allowed_languages.set([self.lang_fr, self.lang_nl])

        with patch.object(Domain, "save", side_effect=AssertionError("domain.save should not be used")):
            serializer._apply_translations(
                domain,
                {
                    "fr": {"name": "FR bis", "description": "Desc FR bis"},
                    "nl": {"name": "NL bis", "description": "Desc NL bis"},
                },
            )

        domain.refresh_from_db()
        self.assertEqual(domain.safe_translation_getter("name", language_code="fr"), "FR bis")
        self.assertEqual(domain.safe_translation_getter("name", language_code="nl"), "NL bis")
