...
class DomainViewSet(
    ...
    def perform_create(self, serializer):
        user = self.request.user
        # Quota check lives on the user model
        # (``CustomUser.can_create_domain``) so the same rule is the
        # source of truth for the API, the frontend topmenu, and the
        # ``domainAccessGuard`` — moving the rule means touching one
        # method, not three callers.
        if not user.can_create_domain():
            owned = Domain.objects.filter(owner=user).count()
            quota = getattr(user, "nb_domain_max", 0) or 0
            raise PermissionDenied(
                f"Domain creation quota exhausted ({owned}/{quota}). "
    ...
    # member-role / moderation-summary / leave moved to DomainMembersActionsMixin.

class DomainJoinRequestViewSet(
    JoinRequestModerationMixin,
    JoinRequestCancelMixin,
    ...
    # leaks into the OpenAPI of unrelated endpoints. Split:
    # moderation actions (approve / reject / bulk-approve / bulk-reject)
    # live in ``JoinRequestModerationMixin`` and go through
    # ``_check_can_approve``; the requester-only ``cancel`` action lives
    # in ``JoinRequestCancelMixin``.
    ...
    # approve / reject / bulk_approve / bulk_reject moved to
    # JoinRequestModerationMixin; cancel moved to
    # JoinRequestCancelMixin. See view_mixins/ for the bodies.
