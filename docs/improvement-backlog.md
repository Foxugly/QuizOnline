...
| O6 | P2 | Réhabiliter les 4 specs e2e fullstack encore en `test.skip` (course-invite, quiz-assign, question ×2, quiz-play). Découvert en câblant O1 : ces specs avaient silencieusement pourri (login `#username`→`#email`, `/api/token/` `username`→`email`, routes `/lms/*` supprimées, `username` retiré du modèle, sélecteurs UI). Auth/routes déjà corrigés. **quiz-assign réhabilité + gate** (clic onglet sessions rendu déterministe, 2026-07-27). Restent 4 `skip`, chacun **avancé + documenté** (1re couche corrigée + note TODO(O6) sur le point de blocage suivant) : course-invite (lien « Voir l'invitation »), question-média (URL `question_media`), question-édition (réponses passées à `app-block-list-editor` = réécriture), quiz-play (flux play/finish). Gatants : smoke ×2 + quiz-assign = **3** (de 2). | `quizonline-frontend/e2e/fullstack/*.spec.ts` | 🔄 (2/5 verts) |
