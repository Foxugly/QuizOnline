...
| P4 | P3 | Pas d'avis/notes apprenants sur les cours (pas de preuve sociale ni boucle de feedback). | `course/models.py` | 🔄 **P4a backend fait** (app `review` : `CourseReview` 1/user/cours, éligibilité=cours terminé, config domaine `reviews_allow_rating`/`_comment`, endpoints list/upsert/delete + modération owner/manager, agrégat sur course-detail, admin, testé). **Reste P4b** : UI (form+liste+étoiles sur course-detail, cases config domaine, ratings sur cartes catalogue via annotation). |
