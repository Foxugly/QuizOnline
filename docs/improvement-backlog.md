...
| O2 | P2 | Aucune mesure ni seuil de couverture en CI (`run-coverage.bat` jamais appelé). | `.github/workflows/ci.yml` | ✅ (backend : `coverage run -m pytest` + `--fail-under=82` (actuel ~85%) ; frontend : config `coverage` angular.json, généré exclu, seuils 33/28/15/34 (actuel ~37/32/17/38) via `npm run test:coverage`) |
| O5 | P3 | Le hook pre-commit `check-i18n` ne se déclenche que sur `shared/i18n/` et `*.i18n.ts` ; une clé ajoutée ailleurs n'est gardée qu'en CI. | `.pre-commit-config.yaml:22` | ✅ (portée élargie à `.i18n.(ts\|json)` → les catalogues JSON par page sont maintenant gardés localement) |
| O6 | P2 | Réhabiliter les specs e2e fullstack. **course-invite + quiz-assign réhabilités + gate** (2026-07-27). **quiz-play à 1 assertion du vert** (parcours complet OK, seule la persistance de la réponse Q2 échoue). **question-média + question-édition = réécritures** (modèles changés : média→blocs `lms/blocks/img/`+`video_url` et le seed ne crée plus de blocs média ; réponses→`app-block-list-editor`). Gatants : smoke ×2 + quiz-assign + course-invite = **4** (de 2 au départ). | `quizonline-frontend/e2e/fullstack/*.spec.ts` | 🔄 (2/4 specs réhabilitées, quiz-play quasi) |
...
| FE3 | P2 | Typage aux frontières API : `any` dans `quiz-question` (composant de jeu principal), `http.get` brut hors client généré dans `lesson-view`, casts `as … & {…}` pour des champs absents du DTO. | `components/quiz-question/quiz-question.ts:206`, `pages/lesson-view/lesson-view.ts:34`, `pages/domain/list/domain-list.ts:231` | 🔄 (quiz-question `any`→`LocalizedQuestionTranslationDto` ; cast compteurs domain-list retiré (DTO les expose). **FE3b évalué 2026-07-27 → PAS un clean win, abandonné** : `completed` est **client-only** (absent du serializer `LessonDetailSerializer`, donc pas de « schéma à rendre honnête ») ; `BlockDto`↔`ContentBlock` diffèrent par les types d'enum → adopter le DTO généré imposerait un cast (ce que FE3 veut éviter) ou un réalignement transverse de `ContentBlock`. Valeur marginale, l'interface locale est un choix délibéré. Reste ouvert seulement si on refait `ContentBlock`=DTO généré un jour. |
| FE4 | P2 | Patterns de recherche divergents entre listes (rechargement serveur / filtrage client / debounce) → comportement incohérent, maintenance dispersée. | `domain-list` vs `quiz-list`/`user-list` vs `subject-list`/`course-list`/`catalog` | 🔄 réévalué : le cas impactant (domain-list = 1 requête/frappe) est **réglé par FE1** (debounce). Le reste = filtrage **client** (quiz-list/user-list, sans requête réseau) → divergence bénigne. Extraction d'un helper partagé = cosmétique, non prioritaire. |
...
| P4 | P3 | Pas d'avis/notes apprenants sur les cours (pas de preuve sociale ni boucle de feedback). | `course/models.py` | ✅ **P4 complet** (backend app `review` + UI) : section avis sur course-detail (agrégat étoiles + liste + form note/commentaire gaté par la config, modération hide/unhide owner/manager), 2 toggles config dans domain-edit (autoriser notes/commentaires). Éligibilité=cours terminé. Reste optionnel : ratings sur les **cartes catalogue** (nécessite une annotation sur la course-list pour éviter le N+1). |
...
| # | Prio | Constat | Référence | État |
|---|------|---------|-----------|------|
| G1 | P1 | App `billing` : modèle sur Domaine (`plan`, `free_until`, `member_count`, prix dérivé), page « Abonnement », tâche beat (rappels + blocage post-deadline), garde d'accès (lecture bloquée hors admin quand suspendu). Paiement manuel (Stripe = phase 2). Livré en tranches. | app `billing`, `domain/models.py`, `config/settings_base.py` (beat) | 🔄 **1a fait** (modèle `DomainBilling` OneToOne, prix `20×(1+⌈membres/100⌉)`, endpoint `GET /api/domain/{id}/billing/` owner/manager, beat quotidien recompte membres + rappels email J-14/J-3 idempotents, admin Django pour poser plan/deadline, défaut `free` sans deadline = jamais bloqué, testé). **1b fait** : page **Abonnement** `/domain/:id/subscription` (owner/manager) — plan, membres, prix mensuel, échéance + callout de statut (payant/gratuit-sans-échéance/gratuit-actif=warn/expiré=danger), action « Abonnement » (icône portefeuille) dans domain-list, i18n 5 langues. **Tranche 2 (blocage) fait** : un domaine dont la deadline gratuite est dépassée est **masqué** de ses membres non-owner/manager via `get_visible_domains` (choke-point de scoping partagé) → verrouille tout le contenu du domaine (lecture+écriture) en un seul endroit ; owner+managers+superuser épargnés ; gaté par `BILLING_ENFORCE_BLOCK` (soupape, défaut True). Testé. **Options faites (2026-07-27)** : bandeau owner cross-app (`app-billing-banner` dans le shell — fetch billing du domaine courant, 403=membre→rien, warn J-14 / danger post-deadline + lien Abonnement) ; auto-switch si le `current_domain` d'un membre devient bloqué (bascule one-shot vers le 1er domaine visible + reload). **Reste** : Stripe = phase 2. |
