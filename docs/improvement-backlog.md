...
| FE3 | P2 | Typage aux frontières API : `any` dans `quiz-question` (composant de jeu principal), `http.get` brut hors client généré dans `lesson-view`, casts `as … & {…}` pour des champs absents du DTO. | `components/quiz-question/quiz-question.ts:206`, `pages/lesson-view/lesson-view.ts:34`, `pages/domain/list/domain-list.ts:231` | 🔄 (quiz-question `any`→`LocalizedQuestionTranslationDto` ; cast compteurs domain-list retiré (DTO les expose). **Reste** : lesson-view sur DTO/service généré = refactor plus profond (réconcilier `blocks: BlockDto[]`↔`ContentBlock[]` + `prev_lesson` + `completed` absent du schéma) ; cast `translations` hand-written↔généré) |
